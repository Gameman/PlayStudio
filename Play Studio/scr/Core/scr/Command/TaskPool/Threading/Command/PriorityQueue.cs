...
namespace Play.Studio.Core.Command.TaskPool.Threading.Command
{
    ...
    /// </summary>
    sealed class PriorityQueue : IEnumerable
    {
        ...
        /// </summary>
        private const int _queuesCount = ThreadPriority.Highest - ThreadPriority.Lowest + 1;

        /// <summary>
        /// Work items queues. There is one for each type of priority
        /// </summary>
        private readonly LinkedList<IHasPriority>[] _queues = new LinkedList<IHasPriority>[_queuesCount];
        ...
        public PriorityQueue()
        {
            for (int i = 0; i < _queues.Length; ++i) {
                _queues[i] = new LinkedList<IHasPriority>();
            }
        }
        ...
        public IHasPriority Dequeue()
        {
            IHasPriority workItem = null;
        ...
        {
            if (_workItemsCount > 0) {
                foreach (LinkedList<IHasPriority> queue in _queues) {
                    queue.Clear();
                }
                _workItemsCount = 0;
                ++_version;
